...
Use `WSUpgradeHeader value` to define the value of the upgrade header that is accepted. Accepted values are
following:

|===
| 2.0 (in development) | 1.3   | Description
| value                | value | protocol name to check before using the WS tunnel (corresponds to ProxyPass upgrade=value)
| *                    | ANY   | read the header value from request
|                      | NONE  | bypass the header check
|===

See mod_proxy_wstunnel (1.3.x and older) and mod_proxy_http (2.x) docs for more information.
