...
For mod_proxy_cluster 1.3.x you have to change the slotmem module.
...
or alternatively you can use CMake:
...
cmake ..
make
cp modules/*so apache_installation_directory/modules
----

Where apache_installation_directory is the location of the installed httpd.
...
A listener must be added in in JWS/Tomcat conf/server.xml.
